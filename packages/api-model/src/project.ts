...
export async function extractProject(opts: {
  ...
  if (opts.port === 'swift') {
    // Generated by `swift build -emit-symbol-graph` into the port's `-docs`
    // worktree, and untracked there — the remote is public, so it cannot be
    // committed, and it is ignored through that worktree's `info/exclude`
    // rather than deleted. It looks like litter in `git status` and is not:
    // remove it and this port silently yields no symbols.
