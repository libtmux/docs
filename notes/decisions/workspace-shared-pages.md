...
## Ruby and Lua stay outside this mechanism

Both ports joined the site after this migration. Ruby ships its own
released `libtmux-workspace` CLI with its own authored pages; Lua has no
workspace product at all. Neither belongs to the generic, unreleased
native-CLI narrative these shared sources describe, so
`site/src/lib/workspace-shared-slots.ts`'s `KNOWN_PORTS` set stays pinned to
the original 8 and the loader, `gen-mentions.mjs`, and `check-api-links.mjs`
all filter their per-port synthesis against it rather than against every
port `ports.ts` lists. Synthesizing for a port outside `KNOWN_PORTS`
produced dead links from pages describing CLI behavior that port does not
have.
