...
try {
  ...
  for (const path of paths) {
    ...
    const expected = path.startsWith('reference/')
      ? '/en/reference/py/libtmux-session-panes/' : `/en/py/stable/${path}/`
    assert.equal(await switcher.locator('a').first().getAttribute('href'), expected)
    ...
    const summaries = await page.locator('[data-page-switchers] summary').evaluateAll((elements) =>
      elements.map((element) => element.getBoundingClientRect().y))
    assert.equal(summaries.length, 2)
    assert(Math.abs(summaries[0] - summaries[1]) <= 1, `${path}: page and locale controls wrapped apart`)
