...
The native Go CLI uses `--log-level` to filter optional diagnostics and
...
`--log-file` appends UTF-8 JSON lines to a regular file. The CLI checks the
...
The log includes lifecycle records at `info` and script stream text at `debug`.
...
## Native Go load failures

After processing an input, native load retains its observed summary for failure
