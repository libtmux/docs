...
description: "Connect the development Go MCP server to a session loaded by its native workspace CLI."
port: go
product: workspace
sidebar:
...
Inspect the session you loaded with the Go workspace CLI by pointing its MCP
...
```console
$ LIBTMUX_TOOLSETS=inspect ./libtmux-mcp \
...
These MCP options use a single dash. Keep the same tmux executable on `PATH` for
both processes.
...
| Tool | Arguments |
| --- | --- |
| [capture_pane][mcp-source] | `"pane_id"`, `"max_lines"`; optional `"history"` |
...
Use returned pane IDs for capture and snapshots. [wait_for_text][mcp-source]
treats its patterns as literal text unless you enable its `"regex"` option.
Discover the current schema before using cursor or history options.
...
[workspace-source]: https://github.com/libtmux/libtmux-go/blob/56e30d41f0dc0eb32d582b581860d2b07a3c0ddb/workspace/CLI.md
[mcp-source]: https://github.com/libtmux/libtmux-go/blob/56e30d41f0dc0eb32d582b581860d2b07a3c0ddb/mcp/TOOLS.md
