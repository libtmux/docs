...
The local Rust `tmux-workspace load --append` validates the inherited daemon
...
See [native Rust load reporting](../../reference/output/#native-rust-load-reporting)
for retained effects and child exit status when result output fails.

Native load can append [filtered JSON logs](../../reference/output/#native-rust-logging)
