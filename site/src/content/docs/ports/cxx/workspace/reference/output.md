...
## Native C++ load output

Human load publishes its summary and flushes stdout and stderr before terminal
...
Closed event output while building retains completed inputs and borrowed-session
...
`load --log-file PATH` appends one JSON diagnostic record per line. The global
...
The destination must be a regular file. New files receive owner-only
